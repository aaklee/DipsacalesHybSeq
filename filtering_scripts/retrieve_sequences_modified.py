...
helptext = '''Usage:
    python retrieve_sequences.py baitfile.fasta sequence_dir aa/dna/intron/supercontig taxon1,taxon2,taxon3,...
...
include_samples = sys.argv[4].split(',')

sampledir = sys.argv[2]
sample_names = [x for x in os.listdir(sampledir) if (os.path.isdir(os.path.join(sampledir,x)) and not x.startswith('.') and x in include_samples)]
...
print("Retreiving {} - {} (paralogs) genes from {} samples".format(len(target_genes),len(paralogs),len(sample_names)))
